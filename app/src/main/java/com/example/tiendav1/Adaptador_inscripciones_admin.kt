...
class Adaptador_inscripciones_admin(private val lista_inscripciones: MutableList<Inscripcion>,var filtros_check: List<Boolean>) :
    RecyclerView.Adapter<Adaptador_inscripciones_admin.UserViewHolder>(), Filterable {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UserViewHolder {
        val vista_item = LayoutInflater.from(parent.context).inflate(R.layout.item_admin_inscripcion, parent, false)
    ...
    override fun onBindViewHolder(holder: UserViewHolder, posicion: Int) {
        ...
        Glide.with(contexto)
            .load(item_actual.url_usuario)
            .apply(Utilidades.opcionesGlide(contexto))
            .transition(Utilidades.transicion)
            .into(holder.imagen)

        holder.nombre.text = item_actual.nombre_usuario
        holder.fecha.text = item_actual.fecha

        holder.layout.setOnClickListener {
            contexto.startActivity(Intent(contexto,Admin_editar_inscripcion::class.java).putExtra("ID", item_actual.id.toString()))
        }
    ...
    inner class UserViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val imagen: ImageView = itemView.findViewById(R.id.admin_inscripciones_usuario_imagen)
        val nombre: TextView = itemView.findViewById(R.id.admin_inscripciones_usuario_nombre)
        val fecha: TextView = itemView.findViewById(R.id.admin_inscripciones_usuario_fecha)
        val estado_color:ImageView = itemView.findViewById(R.id.admin_inscripciones_usuario_estado_color)
        val estado_texto:TextView = itemView.findViewById(R.id.admin_inscripciones_usuario_estado_texto)
        val layout:ConstraintLayout = itemView.findViewById(R.id.admin_inscripciones_usuario_todo)
